...
// Extracts React from an existing registered widget's component
...
(function () {
  ...
  function getReact() {
    // Pull React from an existing built-in widget's prototype chain
    try {
      var widgets = window.CMS.getWidgets();
      for (var i = 0; i < widgets.length; i++) {
        var control = widgets[i].control;
        if (!control) continue;
        // Walk prototype chain looking for React.Component
        var proto = Object.getPrototypeOf(control);
        while (proto && proto !== Object.prototype) {
          if (proto.constructor && proto.constructor.createElement) {
            return proto.constructor;
          }
          proto = Object.getPrototypeOf(proto);
        }
        // Check if control itself has a __reactFiber or similar
        if (control.prototype && control.prototype.isReactComponent) {
          // Get React from the module that defined this component
          var r = control.__react || control._react;
          if (r) return r;
        }
      }
    } catch(e) {
      console.warn('[vimeo-widget] getWidgets approach failed:', e);
    }

    // Last resort: check for React in require cache via webpack
    try {
      if (typeof __webpack_require__ !== 'undefined') {
        var cache = __webpack_require__.c || {};
        for (var id in cache) {
          var m = cache[id] && cache[id].exports;
          if (m && m.createElement && m.Component && m.useState) return m;
          if (m && m.default && m.default.createElement && m.default.Component) return m.default;
        }
      }
    } catch(e) {}
  ...
  function register() {
    ...
    if (!React) {
      console.error('[vimeo-widget] Could not extract React. Falling back to string widget.');
      // Fallback: plain string input with helper text
      window.CMS.registerWidget(
        'vimeo-browser',
        window.CMS.resolveWidget('string').control,
        window.CMS.resolveWidget('string').preview
      );
      return;
    }

    console.log('[vimeo-widget] React found, registering widget.');
