// Vimeo Browser Widget — Decap CMS 3.x compatible
// Waits for CMS to be ready, then extracts React from it

(function () {

  function init() {
    // Decap 3.x bundles React internally — retrieve it
    var React = window.React
      || (window.CMS && window.CMS._deps && window.CMS._deps.react)
      || null;

    // If React still not found, use a minimal shim via CDN
    if (!React) {
      var script = document.createElement('script');
      script.src = 'https://unpkg.com/react@18/umd/react.development.js';
      script.onload = function () { register(window.React); };
  ...
  function register(React) {
    ...
    var css = `
      ...
      .vb-modal{background:#fff;border-radius:6px;width:760px;max-width:95vw;max-height:85vh;display:flex;flex-direction:column;overflow:hidden}
      ...
      .vb-card{background:#fff;cursor:pointer;transition:background .1s}
    ...
    class VimeoWidget extends React.Component {
      ...
      render() {
        ...
        var {open, query, videos, loading, page, pages, total,
             selTitle, selThumb} = this.state;
        ...
        return e('div', {className:'vb-widget'},
          ...
          val
            ? e('div', {className:'vb-selected'},
                selThumb && e('img', {src:selThumb, alt:''}),
                    ...
                    : videos.map(v =>
                        e('div', {
                          ...
                          e('div',{className:'vb-thumb'},
                            v.thumbnail && e('img',{src:v.thumbnail,alt:v.title}),
  ...
  // Wait for CMS to be ready before registering
